...
class NodeService
{
    ...
    public static function applyAuthNode()
    {
        ...
        if (($role_id = session('user.role_id'))) {
            $where = ['status' => '1'];
            $roleids = Db::name('Role')->whereIn('id', explode(',', $role_id))->where($where)->column('id');
            if (empty($roleids)) {
                return session('user.nodes', []);
            }
            $nodes = Db::name('RoleNode')->whereIn('role_id', $roleids)->column('node');
    ...
    public static function get($nodes = [])
    {
        ...
        $ignore = ['index', 'wechat/review', 'admin/plugs', 'admin/login', 'admin/index', 'api', 'admin/config/file'];
