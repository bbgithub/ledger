...
class Index extends BasicAdmin
{
    ...
    public function pass()
    {
        ...
        if ($this->request->isGet()) {
            ...
            return $this->_form('User', 'user/pass');
        }
        ...
        if (DataService::save('User', ['id' => session('user.id'), 'password' => md5($data['password'])])) {
    ...
    {
        $this->assign([
            'region_name'  => RegionService::fullName(),
        ]);
        if (intval($this->request->request('id')) === intval(session('user.id'))) {
            return $this->_form('User', 'user/info');
        }
